...
$(document).ready(function() {
	...
	// manage brand table
	manageBrandTable2= $("#manageBrandTable2");
	...
	$("#submitBrandForm").unbind('submit').bind('submit', function() {
		...
		var earningsAvailable = $("#revenue").val();
		...
		else if(earningsAvailable<100) {
			$("#brandName").after('<p class="text-danger">Sorry, you cannot withdraw. Minimum amount is Ksh 100/-</p>');
			$('#brandName').closest('.form-group').addClass('has-error');
		}
		else {
		...
		if(brandStatus == "") {
			$("#brandStatus").after('<p class="text-danger">Payment method field is required</p>');
		...
		if(brandName && brandStatus && brandName!= "KSh. 0" && earningsAvailable>=100) {
			var form = $(this);
			// button loading
          ...
          '</div>');
			        ...
			        $('#tableHolder').load('php_action/getTable.php');	
  	  			...
  	  			$.ajax({
				url : 'php_action/fetchCurrentEarnings.php',
				type: 'post',
				...
				success:function(response) {
					...
					if(response.success==true){
						...
						$('#EarningsToDateCardHeader').html("<h1>"+
		    	"<div></div>KSh. "+response.earnings+ 
		    	"</h1>");

						$("#brandName").val('KSh. '+response.earnings);
						$("#revenue").val(response.earnings);
