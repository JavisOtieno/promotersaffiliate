...
$sql = "SELECT id, dealDate, name, product_name, product_profit,status FROM deals";
...
if($result->num_rows > 0) { 
 ...
 while($row = $result->fetch_array()) {
 	...
 	// active 
 	if($row[5] == 1) { 		
 		$paymentStatus = "<label class='label label-success'>Complete</label>";
 	} else if($row[5] == 2) { 		
 		$paymentStatus = "<label class='label label-danger'>Cancelled</label>";
 	} else { 		
 		$paymentStatus = "<label class='label label-warning'>Processing...</label>";
 	} // /else
